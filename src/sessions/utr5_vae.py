...
def build_wandb_logger(config, model):
    ...
    run = wandb.init(
        project = "5utr-VAE",
        notes = "follow up training: skilled-vortex-14",
        config = config_dict,
    )
...
def train(config):
    ...
    for epoch in range(config.epoch):
        ...
        for batch in tqdm(train_dataloader, desc=f"Training Epoch {epoch+1}/{config.epoch}", leave=False):
            ...
            # logs
            train_loss_list.append(loss.mean().item()/config.batch)
            train_kld_list.append(kld.mean().item()/config.batch)
            train_ce_list.append(ce.mean().item()/config.batch)
            global_step += config.batch

            if config.log_flag:
                wandb.log({"train_loss/batch": loss.mean().item()/config.batch, 
                        "train_kld/batch": kld.mean().item()/config.batch, 
                        "train_ce/batch": ce.mean().item()/config.batch, 
        ...
        with torch.no_grad():
            ...
            for val_batch in tqdm(val_dataloader, desc=f"Validation Epoch {epoch+1}/{config.epoch}", leave=False):
                ...
                val_loss_list.append(loss.mean().item()/config.batch)
                val_kld_list.append(kld.mean().item()/config.batch)
                val_ce_list.append(ce.mean().item()/config.batch)
        ...
        if config.save_flag and epoch % config.save_model_epochs == 0 and epoch != 0:
            pt_file = os.path.join(config.save_path, TIME+f"_utr5vae_epoch_{epoch}.pt")
